...
import { ArrowLeft, Plus, AlertTriangle, Clock, CheckCircle, ChevronRight, X, Mail, Link2 } from 'lucide-react'
...
export default function EngagementDetail() {
  ...
  const [activeView, setActiveView] = useState('pipeline') // 'pipeline' | 'list' | 'workplan'

  useEffect(() => { fetchData() }, [id])

  async function fetchData() {
    const [engRes, candRes] = await Promise.all([
      supabase.from('engagements').select(`*, assessment_criteria(*)`)
        .eq('id', id).single(),
  ...
  async function markNotProceeding(ceId, reason, notes) {
    await supabase.from('candidate_engagements').update({
      ...
      not_proceeding_notes: notes,
      not_proceeding_at: new Date().toISOString(),
    }).eq('id', ceId)
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #E2E8F0', padding: '1rem 1.5rem', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 0, marginTop: 12, borderBottom: '1px solid #E2E8F0' }}>
          {[['pipeline', 'Pipeline'], ['list', 'All Candidates'], ['prospects', 'Prospects'], ['workplan', 'Workplan']].map(([v, label]) => (
            <button key={v} onClick={() => setActiveView(v)} style={{
              padding: '6px 16px', fontSize: 13, fontWeight: activeView === v ? 600 : 400,
            ...
            return (
              <div key={stageNum} style={{ flexShrink: 0, width: 240, display: 'flex', flexDirection: 'column' }}>
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#4A5568', marginBottom: 2 }}>
                    {stageInfo.label}
                  </div>
      ...
      {showRejection && (
        <RejectionModal
          ce={showRejection}
          onClose={() => setShowRejection(null)}
          onConfirm={markNotProceeding}
        />
      )}
      ...
      {showAddCandidate && (
        <AddCandidateModal
          ...
          onAdded={() => { setShowAddCandidate(false); fetchData() }}
        />
      )}
    </div>
  )
...
function CandidateCard({ ce, stageNum, onOpen, onReject, onMoveNext }) {
  ...
  return (
    <div style={{
      background: ac ? ac.bg : '#fff',
      border: `1px solid ${ac ? ac.border : '#E2E8F0'}`,
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 6 }}>
        ...
        {aging !== 'normal' && <div style={{ width: 8, height: 8, borderRadius: '50%', background: ac.dot, flexShrink: 0, marginTop: 3 }} title={`Awaiting decision — ${aging === 'red' ? '10+' : '5+'} business days`} />}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 8 }} onClick={e => e.stopPropagation()}>
        {NOT_PROCEEDING_ELIGIBLE_STAGES.includes(stageNum) && (
          <button onClick={onReject} style={{ flex: 1, padding: '4px 8px', fontSize: 11, background: '#FFF5F5', color: '#C53030', border: '1px solid #FED7D7', borderRadius: 6, cursor: 'pointer' }}>
            Not Proceeding
          </button>
        )}
        {stageNum < 10 && (
          <button onClick={onMoveNext} style={{ flex: 1, padding: '4px 8px', fontSize: 11, background: '#EBF8FF', color: '#2B6CB0', border: '1px solid #BEE3F8', borderRadius: 6, cursor: 'pointer' }}>
            Advance →
          </button>
        )}
      </div>
      ...
      <tbody>
        {candidates.map(ce => (
          <tr key={ce.id} onClick={() => onOpen(ce)} style={{ borderBottom: '1px solid #F7FAFC', cursor: 'pointer' }}
            ...
            <td style={{ padding: '10px 12px', color: '#718096', fontSize: 11 }}>
              {ce.candidates?.entry_type === 'converted_from_prospect' ? 'Outreach' : 'Inbound'}
            </td>
            <td style={{ padding: '10px 12px' }}>
...
function CandidateModal({ ce, engagement, onClose, onMoveStage, onReject }) {
  ...
  async function toggleNextStep(step) {
    ...
    await supabase.from('candidate_engagements').update({
      [field]: newVal,
      [timeField]: newVal ? new Date().toISOString() : null,
    }).eq('id', ce.id)
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-end', zIndex: 50 }}>
      <div style={{ background: '#fff', width: 540, height: '100vh', overflowY: 'auto', boxShadow: '-4px 0 24px rgba(0,0,0,0.1)', padding: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0D2B45', margin: 0 }}>{ce.candidates?.full_name}</h2>
            <div style={{ fontSize: 13, color: '#718096', marginTop: 2 }}>{ce.candidates?.current_title} · {ce.candidates?.current_organization}</div>
          </div>
        ...
        {(ce.candidates?.resume_url || ce.candidates?.cover_letter_url) && (
          <Section title="Documents">
            {ce.candidates?.resume_url && <Row label="Resume" value={<a href={ce.candidates.resume_url} target="_blank" rel="noreferrer" style={{ color: '#2B6CB0' }}>Download</a>} />}
            {ce.candidates?.cover_letter_url && <Row label="Cover Letter" value={<a href={ce.candidates.cover_letter_url} target="_blank" rel="noreferrer" style={{ color: '#2B6CB0' }}>Download</a>} />}
          </Section>
        )}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 }}>
      <div style={{ background: '#fff', borderRadius: 12, padding: '2rem', width: 460 }}>
        ...
        <div style={{ background: '#EBF8FF', borderRadius: 8, padding: '10px 14px', marginBottom: 20, fontSize: 12, color: '#2B6CB0', display: 'flex', alignItems: 'center', gap: 8 }}>
          <Mail size={14} />
          A rejection email template will be available to send to the candidate after confirming. (Gmail integration coming in Phase 4.)
        </div>
...
function AddCandidateModal({ engagementId, onClose, onAdded }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    setSaving(true)
    // Upsert candidate (check existing by email)
    let candidateId
    const { data: existing } = await supabase.from('candidates').select('id').eq('email', form.email).single()
    if (existing) {
    ...
    // Check if already on this engagement
    const { data: existingCE } = await supabase.from('candidate_engagements').select('id').eq('candidate_id', candidateId).eq('engagement_id', engagementId).single()
